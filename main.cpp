...
float last_error = 0;
const float LOOP_DELAY_MILLIS = 5; // Wait for 0.005s between motor updates.
                 ...
                 float Kd )
{
  ...
  //float second_term = Kd * ((error_position - error_prev)/(LOOP_DELAY_MILLIS/1000.0));
  //last_error = error_position;
  //return first_term + second_term; // YOUR CODE HERE
  
  float error_position = target - pos;
  float error_derivative = (error_position - last_error) / ((LOOP_DELAY_MILLIS)/1000.0);
  float control_output = (Kp * error_position) + (Kd * error_derivative);
  last_error = error_position;
